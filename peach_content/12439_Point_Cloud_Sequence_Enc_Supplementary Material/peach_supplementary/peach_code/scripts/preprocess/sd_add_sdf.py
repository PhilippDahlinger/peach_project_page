...
def dp_queries_occupancy_3d_boundary(num_queries=512, visualize=False):
    mesh_dataset_file = "../datasets/pc_mango/sd_v1.hdf5"
    output_dataset_file = "../datasets/pc_mango/sd_v1_sdf.hdf5"

    with h5py.File(mesh_dataset_file, "r") as f_in, \
         h5py.File(output_dataset_file, "w") as f_out:

        mesh_faces = f_in["global_data"]["cells"][:]
        ...
        for task_name in tqdm(f_in.keys(), desc="Processing Tasks"):
            ...
            for traj_name in tqdm(trajs_in.keys(), desc=" Trajs", leave=False):
                ...
                for ts in range(num_ts):
                    ...
                    pts_surface = sample_near_boundary(
                        verts, mesh_faces, n_surface
                    )

                    pts_uniform = sample_uniform(
                        bbox_min - 10, bbox_max + 10, n_uniform
                    )

                    query_pts = np.concatenate(
                        [pts_surface, pts_uniform], axis=0
                    )
                    ...
                    # Compute SDF
                    sdf = get_sdf_labels(verts, mesh_faces, query_pts)
                    ...
                    #     verts, mesh_faces, query_pts, sdf=sdf, max_points=5000
                    # )
