...
def dp_queries_sdf_2d(num_queries=512):
    mesh_dataset_file = "../datasets/pc_mango/db_v4.hdf5"
    output_dataset_file = "../datasets/pc_mango/db_v4_sdf.hdf5"

    with h5py.File(mesh_dataset_file, "r") as f_in, \
         h5py.File(output_dataset_file, "w") as f_out:

        faces_mesh     = f_in["global_data"]["cell_indices"]["mesh_faces"][:]
        faces_collider = f_in["global_data"]["cell_indices"]["collider_faces"][:]

        # Precompute once -- faces never change across tasks/trajs/timesteps
        boundary_edges_mesh     = extract_boundary_edges(faces_mesh)
        boundary_edges_collider = extract_boundary_edges(faces_collider)
        ...
        for task_name in tqdm(f_in.keys(), desc="Tasks"):
            ...
            for traj_name in tqdm(trajs_in.keys(), desc=" Trajs", leave=False):
                ...
                mesh     = traj_in["mesh_pos"][:]      # (T, V_a, 2)
                collider = traj_in["collider_pos"][:]  # (T, V_b, 2)
                num_ts   = mesh.shape[0]
                ...
                for ts in range(num_ts):
                    ...
                    verts_b = collider[ts]  # (V_b, 2)

                    all_verts = np.concatenate([verts_a, verts_b], axis=0)
                    ...
                    pts_a     = sample_near_boundary_2d(verts_a, boundary_edges_mesh,     n_surface // 2)
                    pts_b     = sample_near_boundary_2d(verts_b, boundary_edges_collider, n_surface // 2)
                    pts_uniform = sample_uniform_2d(bbox_min - 20, bbox_max + 20, n_uniform)

                    query_pts = np.concatenate([pts_a, pts_b, pts_uniform], axis=0)
                    np.random.shuffle(query_pts)

                    sdf_a = signed_distance_2d(query_pts, verts_a, faces_mesh,     boundary_edges_mesh)
                    sdf_b = signed_distance_2d(query_pts, verts_b, faces_collider, boundary_edges_collider)
                    sdf   = np.stack([sdf_a, sdf_b], axis=1)  # (Q, 2)
                    # take the sign for debug
                    # sdf = np.where(sdf < 0, -1, 1)

                    # visualize_sdf_debug(
                    #     verts_a, faces_mesh,
                    #     verts_b, faces_collider,
                    #     query_pts, sdf,
...
def visualize_sdf_debug(
    ...
    plot_mesh(ax, collider_verts, collider_faces, color='green', alpha=0.8)
