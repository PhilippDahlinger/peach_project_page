...
def map_sphere2_to_sphere1(pos1, pos2, avg_edge_length):
    """
    pos1: (N, 3) node positions of sphere 1
    pos2: (N, 3) node positions of sphere 2
    cells1: (M, 3) cell array of sphere 1

    Returns:
        cells2_remapped: cell array using sphere 1's topology, valid for sphere 2's node positions
        node_mapping: mapping[i] = j means sphere2's node i corresponds to sphere1's node j
    """
    # Step 1: Align sphere 2 to sphere 1 by removing translation
    centroid1 = pos1.mean(axis=0)
    centroid2 = pos2.mean(axis=0)

    pos2_aligned = pos2 - (centroid2 - centroid1)  # shift sphere 2 onto sphere 1

    # Step 2: For each node in sphere 1, find the closest node in aligned sphere 2
    tree = cKDTree(pos2_aligned)
    distances, indices = tree.query(pos1)  # indices[i] = which node in sphere2 matches sphere1's node i

    # Sanity check: distances should be near zero if meshes are truly identical topology
    print(f"Max matching error: {distances.max():.6f}, average edge length: {avg_edge_length:.6f}")

    # then assert
    assert distances.max() < 0.1 * avg_edge_length, \
        f"Matching error {distances.max():.4f} too large vs edge length {avg_edge_length:.4f}"

    # Step 3: indices[i] = j means "sphere2 node j corresponds to sphere1 node i"
    # So to use cells1 for sphere2, we need pos2 reordered so that pos2_reordered[i] == pos2[indices[i]]
    # i.e. the inverse mapping

    node_mapping = indices  # sphere1 node i <-> sphere2 node node_mapping[i]

    # Reorder sphere 2's positions to match sphere 1's node ordering
    pos2_reordered = pos2[node_mapping]

    # cells1 now works directly for pos2_reordered
    return pos2_reordered, node_mapping


def remove_unconnected_node(pos, cells):
    """
    Removes the one node not referenced by any cell.
    Returns cleaned positions and updated cell array with re-indexed nodes.
    """
    num_nodes = len(pos)

    # Find all node indices that appear in cells
    used_nodes = np.unique(cells)  # sorted array of node indices that are referenced

    # Find the unconnected one (should be exactly 1)
    all_nodes = np.arange(num_nodes)
    unused_nodes = np.setdiff1d(all_nodes, used_nodes)

    assert len(unused_nodes) == 1, f"Expected 1 unconnected node, found {len(unused_nodes)}"
    print(f"Removing unconnected node at index {unused_nodes[0]}, position {pos[unused_nodes[0]]}")

    # Remove it from positions
    pos_clean = np.delete(pos, unused_nodes[0], axis=0)

    # Re-index cells: node indices above the removed one shift down by 1
    cells_clean = cells.copy()
    cells_clean[cells > unused_nodes[0]] -= 1

    return pos_clean, cells_clean


def check_no_node_at_center(pos):
    ...
    assert closest_dist > 0.5 * median_dist, f"Node {closest_idx} is suspiciously close to the center!"
    print("OK: No node near the center.")
...
def convert(raw_root: str, output_h5: str, experiments_yaml_rel: str = "meta_data_filtered/experiments_phys_valid.yaml", max_tasks: int = 2, n_even: int = 4, discard_first_ts=False,):
    ...
    ball_cells_map = {}
    ball_pos_map = {} # save first occurence of ball pos
    ball_avg_edge_length_map = {}
    ...
    with h5py.File(output_h5, "w") as out:
        ...
        for task_idx, exp in enumerate(tqdm(experiments, desc="Tasks")):
            ...
            if isinstance(ball, dict):
                ...
                if "gravity_direction" in ball:
                    params.create_dataset("gravity_tilt_deg", data=float(ball["gravity_direction"]["tilt_deg"]))
                    params.create_dataset("gravity_azimuth_deg", data=float(ball["gravity_direction"]["azimuth_deg"]))
            ...
            for s2 in exp.get("stage2_samples", []):
                ...
                if key not in ball_cells_map:
                    # remove center node for reference ball
                    ball_pos, ball_cells = remove_unconnected_node(ball_stack[0], ball_faces)
                    # check there is no center node left
                    check_no_node_at_center(ball_pos)
                    # compute average edge length for this ball type to use as tolerance in the mapping
                    edges = np.vstack([
                        ball_pos[ball_cells[:, 0]] - ball_pos[ball_cells[:, 1]],
                        ball_pos[ball_cells[:, 1]] - ball_pos[ball_cells[:, 2]],
                        ball_pos[ball_cells[:, 0]] - ball_pos[ball_cells[:, 2]],
                    ])
                    avg_edge_length = np.linalg.norm(edges, axis=1).mean()
                    ball_cells_map[key] = ball_cells
                    ball_pos_map[key] = ball_pos  # save first frame's positions for this ball type
                    ball_avg_edge_length_map[key] = avg_edge_length

                # remap ball_stack to the same node ordering across tasks using the first occurrence of this ball type as reference
                reference_pos = ball_pos_map[key]
                avg_edge_length = ball_avg_edge_length_map[key]
                current_pos = ball_stack[0]  # (N, 3) positions for the first frame of this task's ball
                _, node_mapping = map_sphere2_to_sphere1(reference_pos, current_pos, avg_edge_length)  # this will assert if the meshes don't share the same topology
                ball_stack = ball_stack[:, node_mapping, :]  # reorder to match the reference topology
