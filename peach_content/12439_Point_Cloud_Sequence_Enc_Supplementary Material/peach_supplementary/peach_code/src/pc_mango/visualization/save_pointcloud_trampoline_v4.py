...
if __name__ == "__main__":
    node_type_id = "node_types"  # or "color" for older datasets, select accordingly
    cut_length = 50  # or None for full length, select accordingly
    min_x = np.min(np.array([-145.0, -145.0, -145.0]))
    max_x = np.max(np.array([[145.0, 145.0, 145.0]]))
                        ...
                        for traj_idx in traj_indices:
                            for traj_key in sorted(task["trajs"].keys()):
                                if int(traj_key[-3:]) == traj_idx:
                                    ...
                                    out_path = f"output/pointcloud_vis/paper_exp/trampoline/pc_task_{task_id}_traj_{traj_idx}"
