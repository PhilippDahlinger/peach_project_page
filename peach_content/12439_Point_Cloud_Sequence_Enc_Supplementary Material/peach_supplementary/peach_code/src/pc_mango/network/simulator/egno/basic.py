...
class BaseMLP(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, activation, residual=False, last_act=False):
...
class EquivariantScalarNet(nn.Module):
    def __init__(self, n_vector_input, hidden_dim, activation, n_scalar_input=0, norm=True):
        """
        ...
        self.out_vector_net = BaseMLP(self.hidden_dim, self.hidden_dim, n_vector_input, self.activation)
        self.out_scalar_net = BaseMLP(self.hidden_dim, self.hidden_dim, self.output_dim, self.activation)
...
class EGNN_Layer(nn.Module):
    def __init__(self, in_edge_nf, hidden_nf, activation=nn.SiLU(), with_v=False, norm=False,
                 h_update=True):
...
class EGNN(nn.Module):
    def __init__(self, n_layers, in_node_nf, in_edge_nf, hidden_nf, activation=nn.SiLU(), with_v=False,
                 norm=False):
        ...
        for i in range(self.n_layers):
            layer = EGNN_Layer(in_edge_nf, hidden_nf, activation=activation, with_v=with_v, norm=norm)
    ...
    def forward(self, x, h, edge_index, edge_fea, v=None):
        ...
        if reshape:
            x = x.view(T, num_nodes, x.shape[-1])
            if v is not None:
                v = v.view(T, num_nodes, v.shape[-1])
            h = h.view(T, num_nodes, h.shape[-1])
        return (x, v, h) if v is not None else (x, h)


class EGMN(nn.Module):
    def __init__(self, n_layers, n_vector_input, hidden_dim, n_scalar_input, activation=nn.SiLU(), norm=False,
                 ):
        ...
        for i in range(self.n_layers):
            cur_layer = EquivariantScalarNet(n_vector_input=n_vector_input + i, hidden_dim=hidden_dim,
                                             activation=activation,
                                             n_scalar_input=n_scalar_input if i == 0 else hidden_dim,
                                             norm=norm)
...
class GNN_Layer(nn.Module):
    def __init__(self, in_edge_nf, hidden_nf, activation=nn.SiLU(), with_v=False):
        super(GNN_Layer, self).__init__()
        self.with_v = with_v
        self.edge_message_net = BaseMLP(input_dim=in_edge_nf + 2 * hidden_nf, hidden_dim=hidden_nf,
...
class GNN(nn.Module):
    def __init__(self, n_layers, in_node_nf, in_edge_nf, hidden_nf, activation=nn.SiLU()):
        ...
        self.embedding = nn.Linear(in_node_nf, hidden_nf)
        for i in range(self.n_layers):
            layer = GNN_Layer(in_edge_nf, hidden_nf, activation=activation)
            self.layers.append(layer)
