# Optional: torch_scatter for efficient aggregation
try:
    ...
    def scatter_mean(src, index, dim_size=None, **kwargs):
        """Fallback scatter_mean using PyTorch."""
        if dim_size is None:
            dim_size = index.max().item() + 1
        count = torch.zeros(dim_size, device=src.device, dtype=src.dtype)
        count.scatter_add_(0, index, torch.ones_like(src[:, 0] if src.dim() > 1 else src))
        ...
        return result / count.clamp(min=1).unsqueeze(-1) if src.dim() > 1 else result / count.clamp(min=1)

    def scatter_sum(src, index, dim_size=None, **kwargs):
        """Fallback scatter_sum using PyTorch."""
        if dim_size is None:
            dim_size = index.max().item() + 1
        result = torch.zeros((dim_size,) + src.shape[1:], device=src.device, dtype=src.dtype)
        result.scatter_add_(0, index.unsqueeze(-1).expand_as(src) if src.dim() > 1 else index.unsqueeze(1), src)
        return result

    def scatter_max(src, index, dim_size=None, **kwargs):
        """Fallback scatter_max using PyTorch."""
        if dim_size is None:
            dim_size = index.max().item() + 1
        result = torch.full((dim_size,) + src.shape[1:], float('-inf'), device=src.device, dtype=src.dtype)
        index_expanded = index.unsqueeze(-1).expand_as(src) if src.dim() > 1 else index.unsqueeze(1)
        result.scatter_(0, index_expanded, src)
        indices = torch.full((dim_size,) + src.shape[1:], -1, device=src.device, dtype=torch.long)
        indices.scatter_(0, index_expanded, torch.arange(len(src), device=src.device).unsqueeze(-1).expand_as(index_expanded))
        return result, indices

    def scatter_min(src, index, dim_size=None, **kwargs):
        """Fallback scatter_min using PyTorch."""
        if dim_size is None:
            dim_size = index.max().item() + 1
        result = torch.full((dim_size,) + src.shape[1:], float('inf'), device=src.device, dtype=src.dtype)
        index_expanded = index.unsqueeze(-1).expand_as(src) if src.dim() > 1 else index.unsqueeze(1)
    ...
    def scatter_std(src, index, dim_size=None, **kwargs):
        """Fallback scatter_std using PyTorch."""
        if dim_size is None:
            dim_size = index.max().item() + 1
        mean = scatter_mean(src, index, dim_size)
        # Compute variance
        mean_centered = src - mean[index]
        var = scatter_mean(mean_centered ** 2, index, dim_size)
        return torch.sqrt(var.clamp(min=0))

    if not HAS_TORCH_SCATTER:
        print("Warning: torch_scatter not available, using fallback scatter functions (slower)")
