...
def add_noise_to_mesh_nodes(data: Data, sigma: float, device):
    ...
    if sigma > 0.0:
            ...
            # also noise velocity
            first_vel_col = data.x_description[0].index("vel")
            assert first_vel_col is not None, "Velocity features not found in data.x_description"
            data.x[indices, first_vel_col:first_vel_col+num_noise_features] = data.x[indices, first_vel_col:first_vel_col+num_noise_features] + noise
    return data
