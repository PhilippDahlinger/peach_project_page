...
# Optional: pymesh for mesh IoU evaluation (not required for training)
try:
    import pymesh
    HAS_PYMESH = True
except ImportError:
    HAS_PYMESH = False
    print("Warning: pymesh not available. Mesh IoU evaluation will be skipped.")

from pc_mango.ggns.src.utils.dataset_utils import build_dataset_for_split, get_mesh_triangles_from_sofa
from pc_mango.ggns.src.utils.train_utils import add_noise_to_pcd_points
from pc_mango.ggns.src.utils.get_connectivity_setting import get_radius_dict
from pc_mango.ggns.src.utils.data_utils import get_timestep_data, convert_to_hetero_data, get_shortest_trajectory, \
...
def calculate_mesh_iou(target_mesh_nodes, predicted_mesh_nodes, target_triangles, predicted_triangles):
    if not HAS_PYMESH:
        print("Warning: pymesh not available. Skipping mesh IoU calculation.")
        return 0.0
