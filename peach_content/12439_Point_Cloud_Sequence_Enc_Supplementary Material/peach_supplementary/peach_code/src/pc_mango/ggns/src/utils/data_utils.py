...
from pc_mango.ggns.src.utils.graph_utils import add_static_tissue_info
from pc_mango.ggns.util.Types import *


def get_timestep_data(data: Data, predicted_position: Tensor, input_timestep: str, use_color: bool, use_poisson=False,
                      tissue_task=False):
    """
...
def convert_to_mgn_hetero(data: Data, device, tissue_task: bool, use_mesh_coordinates: bool) -> HeteroData:
    ...
    hetero_data.node_stores[0].x = torch.cat(
        (hetero_data.node_stores[0].x, build_one_hot_features_batched(device, old_node_type)), dim=1)
    ...
    if tissue_task:
        ...
        hetero_data.node_stores[0].x = add_static_tissue_info_batched(device, hetero_data.node_stores[0].x, data.ptr,
                                                                      old_node_type)
...
def convert_to_hetero_data(data: Data, hetero: bool, use_color: bool, device, tissue_task: bool, use_world_edges: bool,
                           use_mesh_coordinates: bool, mgn_hetero=False):
    """
    ...
    if hetero:
        ...
        if mgn_hetero:
            ...
            if use_color:
                hetero_data.node_stores[0].x = torch.cat((hetero_data.node_stores[0].x, data.grid_colors.float()),
                                                         dim=1)
                collider_velocity = torch.ones(hetero_data.node_stores[1].x.shape[0]).to(device) * (-200.0 / 100 * 0.01)
                hetero_data.node_stores[1].x = torch.cat((hetero_data.node_stores[1].x, collider_velocity.view(-1, 1)),
                                                         dim=1)
            if tissue_task:
                # add gripper velocity to gripper nodes
                hetero_data.node_stores[1].x = torch.cat(
                    (hetero_data.node_stores[1].x, data.collider_velocities.view(-1, 3)), dim=1)
                # add static mesh nodes info to mesh nodes
            ...
            if use_mesh_coordinates:
                mesh_attr = get_relative_mesh_positions_batched(device, data.mesh_edges,
                                                                data.initial_mesh_positions).to(device)
                hetero_data.edge_stores[2].edge_attr = torch.cat((hetero_data.edge_stores[2].edge_attr, mesh_attr),
                                                                 dim=1)
    else:
        hetero_data = data
